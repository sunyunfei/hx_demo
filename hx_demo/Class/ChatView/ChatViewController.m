...
- (void)setupBarButtonItem
{
    ...
    //浮层顶部视图
    UIView *topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.frame), 55)];
    ...
    UIButton *clearButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.view.frame)- 44, 0, 44, 44)];
    ...
    //显示标题
    UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake((CGRectGetWidth(self.view.frame) - 100) / 2, (55 - 20) / 2, 100, 20)];
...
{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 55, self.view.frame.size.width, self.view.frame.size.height - self.chatToolBar.frame.size.height - 55) style:UITableViewStylePlain];
    ...
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (cell == nil) {
                ...
                //对话头像的设置
                if (model.isSender) {
                    cell.headImageView.image = [UIImage imageNamed:@"people1.jpg"];
                }
                else{
                    cell.headImageView.image = [UIImage imageNamed:@"customer1.jpg"];
                }
            }
...
- (NSDictionary*)getUserInfoAttribute
{
    ...
    [visitor setObject:@"未填写" forKey:@"trueName"];
    ...
    [visitor setObject:nickname.length==0?@"未填写":nickname forKey:@"userNickname"];
    //这个属性必须有，确定是售前售后，还是第三种选择
    ext = @{@"visitor":visitor};
    return ext;
}
